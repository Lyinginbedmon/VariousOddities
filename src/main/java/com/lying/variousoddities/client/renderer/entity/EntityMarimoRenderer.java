...
public class EntityMarimoRenderer extends EntityRenderer<EntityMarimo>
{
	...
	public void render(EntityMarimo entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn)
	{
		matrixStackIn.pushPose();
		 	float[] color = entityIn.getColor().getTextureDiffuseColors();
		 	float ageInTicks = handleRotationFloat(entityIn, partialTicks);
			float renderYaw = Mth.lerp(partialTicks, entityIn.yBodyRotO, entityIn.yBodyRot);
			float headYaw = Mth.lerp(partialTicks, entityIn.yHeadRotO, entityIn.yHeadRot);
			...
			this.entityModel.setupAnim(entityIn, 0F, 0F, ageInTicks, entityYaw, Mth.lerp(partialTicks, entityIn.xRotO, entityIn.getXRot()));
